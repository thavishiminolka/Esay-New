...
export default function EditExams() {
  ...
  useEffect(() => {
    if (!exam || !exam.id) {
      showNotification(
        "error",
        "No exam data provided. Please select an exam to edit."
      );
      setTimeout(() => navigate("/viewExams"), 3000);
      return;
    }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    if (!exam || !exam.id) {
      showNotification(
        "error",
        "Invalid exam ID. Please select an exam to edit."
      );
      setIsSubmitting(false);
      setTimeout(() => navigate("/viewExams"), 3000);
      return;
    ...
    try {
      ...
      setTimeout(() => navigate("/viewExams"), 2000);
    } catch (error: unknown) {
      console.error("Error updating exam:", error);
      const errorMessages: string[] = [];
      if (error instanceof Error) {
