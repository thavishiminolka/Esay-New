...
export default function ScheduleExams() {
  ...
  const validateForm = (): string[] => {
    ...
    if (!topic.trim()) {
      errors.push("Exam topic is required");
    }
    const totalMinutes =
    ...
    if (parseInt(durationHours) > 23) {
      errors.push("Hours cannot exceed 23");
    }
    if (parseInt(durationMinutes) > 59) {
    ...
    try {
      ...
      questions.forEach((q, index) => {
        ...
        questionInfo?.answers.forEach((answer, ansIndex) => {
          if (answer.inputType === "photo" && answer.photo) {
