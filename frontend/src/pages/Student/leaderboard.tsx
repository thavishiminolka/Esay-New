    ...
    const fetchPapersAndLeaderboard = async () => {
      try {
        ...
        if (!papersResponse.ok) {
          throw new Error(
            `Failed to fetch papers: ${papersResponse.status} ${papersResponse.statusText}`
          );
        }
        ...
        if (!Array.isArray(papersData)) {
          throw new Error(
            "Expected an array of exams, but received: " +
              JSON.stringify(papersData)
          );
        }
      ...
      const fetchLeaderboard = async () => {
        try {
          ...
          const response = await fetch(
            `http://localhost:5000/api/results/leaderboard?examId=${selectedPaper}`,
            {
              credentials: "include",
            }
          );
          ...
          if (!Array.isArray(data)) {
            throw new Error(
              "Expected an array of leaderboard data, but received: " +
                JSON.stringify(data)
            );
          }
