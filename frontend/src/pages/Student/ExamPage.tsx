...
//     const saved = localStorage.getItem(`exam-${id}-currentQuestionIndex`)
//     return saved ? Number.parseInt(saved, 10) : 0
//   })
//   const [userAnswers, setUserAnswers] = useState<{ [key: string]: string }>(() => {
//     const saved = localStorage.getItem(`exam-${id}-userAnswers`)
//     return saved ? JSON.parse(saved) : {}
//   })
//   const [isFinished, setIsFinished] = useState(() => {
//     const saved = localStorage.getItem(`exam-${id}-isFinished`)
//     return saved === "true"
//   })
...
//   const [examPhase, setExamPhase] = useState<"reading" | "listening" | "finished">(() => {
//     const saved = localStorage.getItem(`exam-${id}-phase`)
...
//         }
//         data.questions.sort((a: Question, b: Question) => a.questionNumber - b.questionNumber)
//         setExam(data)
//         const readingMinutes = Number.parseInt(data.readingTimeMinutes || "0", 10)
//         const listeningMinutes = Number.parseInt(data.listeningTimeMinutes || "0", 10)
//         setReadingTimeSeconds(isNaN(readingMinutes) ? 0 : readingMinutes * 60)
//         setListeningTimeSeconds(isNaN(listeningMinutes) ? 0 : listeningMinutes * 60)
//         setLoading(false)
//       } catch (err) {
...
//   useEffect(() => {
//     if (!isFinished && id && examPhase === "reading") {
//       localStorage.setItem(`exam-${id}-currentQuestionIndex`, currentQuestionIndex.toString())
//     }
//   }, [currentQuestionIndex, id, isFinished, examPhase])

//   useEffect(() => {
//     if (!isFinished && id) {
//       localStorage.setItem(`exam-${id}-userAnswers`, JSON.stringify(userAnswers))
//     }
//   }, [userAnswers, id, isFinished])

//   useEffect(() => {
//     if (isFinished && exam && id) {
//       localStorage.setItem(`exam-${id}-isFinished`, "true")
//       localStorage.removeItem(`exam-${id}-currentQuestionIndex`)
//       localStorage.removeItem(`exam-${id}-userAnswers`)
//       localStorage.removeItem(`exam-${id}-reading-startTime`)
//       localStorage.removeItem(`exam-${id}-listening-startTime`)
//       exam.questions.forEach((question: Question) => {
//         localStorage.removeItem(`question-${question._id || question.questionNumber}-playCount`)
//       })
//     }
//   }, [isFinished, id, exam])

//   useEffect(() => {
//     if (id && examPhase && exam) {
//       localStorage.setItem(`exam-${id}-phase`, examPhase)
//       if (examPhase === "listening") {
//         const firstListeningIndex = exam.questions.findIndex((q) => q.type === "listening")
//         if (firstListeningIndex !== -1 && currentQuestionIndex < firstListeningIndex) {
...
//       }
//     }
//   }, [examPhase, id, exam, currentQuestionIndex])
...
//       if (id && !isFinished) {
//         localStorage.setItem(`exam-${id}-userAnswers`, JSON.stringify(newAnswers))
//       }
//       return newAnswers
//     })
//   }
...
//         setCurrentQuestionIndex(firstListeningIndex)
//         localStorage.setItem(`exam-${id}-phase`, "listening")
//         localStorage.setItem(`exam-${id}-listening-startTime`, Date.now().toString())
...
//               setExamPhase("finished")
//               setIsFinished(true)
//               showDialog("Exam Completed", "No listening questions available. Exam completed.")
//             }
//           },
//           "Proceed",
//           "Stay in Reading",
...
//         } else {
//           showDialog("Reading Section Incomplete", "Please answer all reading questions before proceeding.")
//         }
//       }
//     }
//   }
...
//       showDialog("Navigation Restricted", "You can only access reading questions during the reading phase.")
//       return
//     }
//     if (examPhase === "listening") {
//       showDialog("Navigation Restricted", "Navigation is automatic in the listening section.")
//       return
//     }
...
//     if (!exam) return
//     console.log("handleAudioComplete called, currentQuestionIndex:", currentQuestionIndex)
...
//     const answerIndex = question.answers.findIndex((ans) => (ans.text || ans.photo || "") === selectedAnswer)
...
//         <h2 className="text-2xl font-bold text-red-500">No Questions Available</h2>
//         <p className="text-gray-600 mt-2">This exam does not contain any questions.</p>
...
//         <ExamResult questions={exam.questions} userAnswers={userAnswers} />
//         <Link to={"/exams"}>
...
//                 Duration: {exam.duration.includes(":") ? exam.duration : `${exam.duration}`}
//               </p>
...
//                       ? (readingQuestions.filter((q) => userAnswers[q._id!]).length / readingQuestions.length) * 100
//                       : 0
//                   }%`,
//                 }}
//               ></div>
...
//                       ? (listeningQuestions.filter((q) => userAnswers[q._id!]).length / listeningQuestions.length) * 100
//                       : 0
//                   }%`,
//                 }}
//               ></div>
...
//               Question {currentQuestion?.questionNumber || currentQuestionIndex + 1} of {exam.questions.length}
//               <span className="ml-2 text-sm font-normal text-gray-500">
//                 ({currentQuestion?.type === "reading" ? "Reading" : "Listening"} Section)
//               </span>
//             </h2>
//             {currentQuestion && (
//               <ExamQuestion
...
//               >
//                 {readingQuestions.every((q) => userAnswers[q._id!]) ? "Proceed to Listening" : "Next"}
//               </button>
//             </div>
//           )}
//         </div>
...
//                 <p className="text-sm text-blue-600">Time: {formatTime(readingTimeSeconds)}</p>
//               </div>
...
//                   const questionIndex = exam.questions.findIndex((q) => q._id === question._id)
//                   const hasAnswer = !!userAnswers[question._id!]
//                   return (
//                     <div key={question._id || questionIndex} className="grid grid-cols-2 gap-x-4 items-center">
//                       <button
...
//             <div className="bg-gray-100 rounded-md p-3 mb-3">
//               <div className="flex justify-between items-center">
//                 <h4 className="font-semibold text-gray-700">Listening Section</h4>
//                 <p className="text-sm text-green-600">Time: {formatTime(listeningTimeSeconds)}</p>
//               </div>
//               <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-2 mt-2">
//                 <div className="text-sm font-medium text-gray-600">Question</div>
//                 <div className="text-sm font-medium text-gray-600">Your Answer</div>
...
//                 : `${exam.questions.length - Object.keys(userAnswers).length} questions remaining`}
//             </p>
//           </div>
//         </div>
...
//           </p>
//           <p className="text-sm text-gray-600 mb-2">
//             <span className="font-medium">Answered:</span> {Object.keys(userAnswers).length}
//           </p>
//           <p className="text-sm text-gray-600">
//             <span className="font-medium">Current:</span> Question {currentQuestionIndex + 1}
//           </p>
//         </div>
//       </div>
//     </div>
      ...
      const saveResultToDatabase = async () => {
        ...
        const resultData = {
          ...
          questions: questions.map((q) => ({
            questionId: q._id!,
            userAnswer: userAnswers[q._id!] || "",
            correctAnswer: q.answers.find((ans) => ans.isCorrect)?.text || q.answers.find((ans) => ans.isCorrect)?.photo || "",
            isCorrect: userAnswers[q._id!] && q.answers.find((ans) => ans.isCorrect)
              ? userAnswers[q._id!].trim().toLowerCase() === (q.answers.find((ans) => ans.isCorrect)?.text || q.answers.find((ans) => ans.isCorrect)?.photo || "").trim().toLowerCase()
              : false,
          })),
          totalScore,
          totalPossibleMarks,
...
>>>>>>> Stashed changes
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (!isFinished && examId && exam && Object.keys(userAnswers).length > 0) {
        e.preventDefault();
        e.returnValue = "Your exam results will not be saved if you leave. Are you sure?";
      }
    };
