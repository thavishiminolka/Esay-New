...
const ExamPage: React.FC = () => {
  ...
  useEffect(() => {
    if (isFinished && examId && exam && !hasSaved) {
      console.log("Running saveResultToDatabase", {
        isFinished,
        examId,
        exam,
        hasSaved,
      ...
      const saveResultToDatabase = async () => {
        ...
        if (!questions || questions.length === 0) {
          console.log("No questions available to save.");
          showNotification("error", "No questions available to save results.");
          return;
        }
        ...
        try {
          ...
          showNotification(
            "success",
            `Result saved successfully! Attempt ${
              data.result?.attempt || "unknown"
            }`
          );
          setHasSaved(true);
        } catch (error: unknown) {
    ...
    if (examPhase === "reading") {
      ...
      if (readingQuestions.every((q) => userAnswers[q._id!])) {
        showNotification(
          ...
          () => removeNotification("proceed-listening")
        );
      } else {
        const nextReadingIndex = exam.questions.findIndex(
          (q, idx) => idx > currentQuestionIndex && q.type === "reading"
  ...
  return (
    <div className="container mx-auto p-4 flex flex-col lg:flex-row gap-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-3xl font-bold text-custom-blue2 font-ubuntu">
            {exam.topic}
          </h1>
          <div className="text-right">
            <div className="flex flex-col gap-1">
        ...
        <div className="flex-1 bg-white rounded-lg border border-gray-300 p-6 lg:p-8">
          <div className="border-t border-gray-200 pt-4">
            <h2 className="text-lg font-semibold text-gray-700 mb-4 font-ubuntu">
              Question{" "}
              {currentQuestion?.questionNumber || currentQuestionIndex + 1} of{" "}
              {exam.questions.length}
              <span className="ml-2 text-sm font-normal text-gray-500">
      ...
      <div className="w-full lg:w-80 xl:w-96 bg-white rounded-lg border border-gray-300 p-6 shadow-sm">
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 font-ubuntu">
            Question Navigator
          </h3>
